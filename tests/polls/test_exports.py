...
def test_poll_comment_export_view(poll, comment_factory, rating_factory):
    ...
    assert header == ['ID', 'Comment', 'Created', 'Link', 'Creator',
    ...
    assert comment_export_view.get_field_data(comment_1, 'ratings_positive') \
           == 1
    assert comment_export_view.get_field_data(comment_1, 'ratings_negative') \
           == 0
    assert \
        comment_export_view.get_field_data(comment_2, 'ratings_negative') == 1
    assert \
