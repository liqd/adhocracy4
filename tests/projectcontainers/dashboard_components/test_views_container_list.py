...
def test_dashboard_container_list_view(
    ...
    url = reverse(
        "a4dashboard:container-list", kwargs={"organisation_slug": organisation.slug}
    )
    client.login(username=initiator.email, password="password")
    response = client.get(url)
    assert_template_response(
        response, "meinberlin_projectcontainers/container_list_dashboard.html"
    )
...
@pytest.mark.django_db
def test_dashboard_container_list_view_user(user, organisation, client):
