...
def test_edit_view(client, project, module_factory, phase_factory, admin):
    ...
    assert module.phase_set.get(id=phase_0.id).name == data.get("phase_set-0-name")
    assert module.phase_set.get(id=phase_0.id).description == data.get(
        "phase_set-0-description"
    )
    assert module.phase_set.get(id=phase_1.id).name == data.get("phase_set-1-name")
    assert module.phase_set.get(id=phase_1.id).description == data.get(
        "phase_set-1-description"
    )


@pytest.mark.django_db
def test_edit_view_validation(client, project, module_factory, phase_factory, admin):
    module = module_factory(project=project)
    phase_0 = phase_factory(module=module)
    phase_1 = phase_factory(module=module)
    url = component.get_base_url(module)
    ...
    assert {
        "end_date": [
            "Phases cannot run at the same time and must follow " "after each other."
        ]
    } in errors


@pytest.mark.django_db
def test_edit_view_validation_2(client, project, module_factory, phase_factory, admin):
    module = module_factory(project=project)
    phase_0 = phase_factory(module=module)
    phase_1 = phase_factory(module=module)
    url = component.get_base_url(module)
    ...
    response = client.post(url, data)
    errors = response.context_data["form"].errors
    assert {"end_date": ["End date can not be before start date."]} in errors


@pytest.mark.django_db
def test_edit_view_validation_3(client, project, module_factory, phase_factory, admin):
    module = module_factory(project=project)
    phase_0 = phase_factory(module=module, weight=1)
    phase_1 = phase_factory(module=module, weight=2)
    url = component.get_base_url(module)
    client.login(username=admin.username, password="password")
    response = client.get(url)
    assert_template_response(response, "a4dashboard/base_form_module.html")
    ...
    response = client.post(url, data)
    errors = response.context_data["form"].errors
    assert {"start_date": ["Phases need to be in same order as in form."]} in errors
