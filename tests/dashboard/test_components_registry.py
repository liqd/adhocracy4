...
def test_register(dashboard_test_component_factory):
    project_component0 = dashboard_test_component_factory(weight=0, identifier="b")
    project_component1 = dashboard_test_component_factory(weight=0, identifier="a")
    ...
    assert components.projects == {"b": project_component0, "a": project_component1}
    assert components.modules == {"c": module_component}

    assert components.get_project_components() == [
        project_component1,
        project_component0,
    ]
...
def test_replace(dashboard_test_component_factory):
    ...
    component0 = dashboard_test_component_factory(identifier="test", label="original")
    component1 = dashboard_test_component_factory(identifier="test", label="overwrite")
...
def test_urls(dashboard_test_component_factory):
    ...
    assert any(
        map(lambda urlpattern: urlpattern.resolve("pc-url-pattern"), urlpatterns)
    )
    assert any(
        map(lambda urlpattern: urlpattern.resolve("mc-url-pattern"), urlpatterns)
    )
