...
class ServiceKontoResponse:
    ...
    def _dict_to_string(cls, d, upper=True):
        return " ".join(
            '{}="{}"'.format(key.upper() if upper else key, value)
            for key, value in d.items()
        )

    def __str__(self):
        ctx = dict(
            hhgw=self._dict_to_string(self.hhgw),
            roles=self._dict_to_string(self.roles),
            authentication=self._dict_to_string(self.authentication, False),
        )
        return (
            "<USERDATA><HHGW {hhgw} /><ROLES {roles} />"
...
def test_connect_account(monkeypatch, client, user):
    ...
    service_konto = ServiceKontoResponse(hhgw={"email": user.email})
    _mock_servicekonto_response(monkeypatch, service_konto)

    response = client.post(reverse("servicekonto_callback"), {"Token": "1234567"})
    assert response.status_code == 302

    account = SocialAccount.objects.get(user=user, provider=ServiceKontoProvider.id)
...
def test_login(monkeypatch, client, social_account):
    _session_state(
        client, {"process": AuthProcess.LOGIN, "next": "/login_session_successful"}
    )
    user = social_account.user

    service_konto = ServiceKontoResponse(
        hhgw={"email": user.email, "userid": social_account.uid}
    )
    _mock_servicekonto_response(monkeypatch, service_konto)
...
@pytest.mark.django_db
def test_error_on_login_with_existing_username(
    monkeypatch, client, user_factory, settings
):
    settings.LOGIN_REDIRECT_URL = "/login_successful"
    ...
    assert_template_response(response, "socialaccount/signup.html")

    assert not User.objects.filter(email=service_konto.hhgw["email"]).exists()


@pytest.mark.django_db
def test_error_on_login_with_existing_email(
    monkeypatch, client, user_factory, settings
):
    settings.LOGIN_REDIRECT_URL = "/login_successful"
    ...
    with pytest.raises(ObjectDoesNotExist):
        SocialAccount.objects.get(
            uid=service_konto.hhgw["userid"], provider=ServiceKontoProvider.id
        )


@pytest.mark.django_db
def test_on_on_login_no_terms_of_use(monkeypatch, client, user_factory, settings):
    settings.LOGIN_REDIRECT_URL = "/login_successful"
    ...
    response = client.post(
        "/accounts/social/signup/",
        {"email": service_konto.hhgw["email"], "username": "username"},
    )
    # return to the form with an error message
    assert_template_response(response, "socialaccount/signup.html")
...
def test_invalid_token(monkeypatch, client, user):
    ...
    response = client.post(reverse("servicekonto_callback"), {"Token": "invalidtoken"})
