...
def test_valid_point_correctly_serialized_to_internal_value(geojson_point, gis_point):
    ...
    assert data["point"].equals(gis_point)
    assert "street_name" not in data


@pytest.mark.django_db
def test_valid_point_with_properties_to_internal_value(geojson_point, gis_point):
    serializer = TestPointSerializer()
    data = serializer.to_internal_value(data={"point": json.dumps(geojson_point)})
    assert data["point"].equals(gis_point)
...
@pytest.mark.django_db
def test_valid_point_to_representation(project, geojson_point, gis_point):
    project.point = gis_point
