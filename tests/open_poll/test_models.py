...
from adhocracy4.open_poll.models import Choice
from adhocracy4.open_poll.models import Question


@pytest.mark.django_db
def test_question_choice_list(user, question, vote_factory, choice_factory):
...
@pytest.mark.django_db
def test_user_answer(open_question, user_factory, answer_factory):

    user1 = user_factory()
    user2 = user_factory()
    anonymous = AnonymousUser()

    answer = answer_factory(creator=user1, answer="user answer", question=open_question)
...
def test_question_queryset(
    ...
    question1 = question_factory(poll=open_poll, label="question1")
    choice1_1 = choice_factory(question=question1)

    question2 = question_factory(
        poll=open_poll, multiple_choice=True, label="question2"
    )
    ...
    question3 = open_question_factory(poll=open_poll, label="question3")
...
@pytest.mark.django_db
def test_question_clean(question, choice_factory):
    ...
    with pytest.raises(ValidationError) as error:
        question.save()
    assert error.value.messages[0].startswith(
        "Question with choices cannot become open question."
    )
...
@pytest.mark.django_db
def test_answer_clean(question, open_question, answer_factory):

    with pytest.raises(ValidationError) as error:
        answer_factory(question=question)
    assert error.value.messages[0] == "Only open questions can have answers."

    answer_factory(question=open_question)


@pytest.mark.django_db
def test_choice_queryset(question, user_factory, vote_factory, choice_factory):
...
@pytest.mark.django_db
def test_choice_clean(question, choice_factory):

    with pytest.raises(ValidationError) as error:
        choice_factory(question=question, is_other_choice=True)
    assert error.value.messages[0].startswith('"Other" cannot be the only choice.')

    question.is_open = True
    question.save()
    with pytest.raises(ValidationError) as error:
        choice_factory(question=question, is_other_choice=True)
    assert error.value.messages[0].startswith("Open questions cannot have choices.")
    ...
    with pytest.raises(ValidationError) as error:
        choice_factory(question=question, is_other_choice=True)
    assert error.value.messages[0] == 'Question already has "other" choice.'


@pytest.mark.django_db
def test_multi_choice_queryset(question, user_factory, vote_factory, choice_factory):
...
@pytest.mark.django_db
def test_other_vote_clean(vote, vote_on_other, other_vote_factory):
...
@pytest.mark.django_db
def test_get_absolute_url(open_poll, question, vote, answer, choice, other_vote):

    assert open_poll.get_absolute_url() == open_poll.project.get_absolute_url()
    assert question.get_absolute_url() == question.poll.get_absolute_url()
    assert answer.get_absolute_url() == answer.question.poll.get_absolute_url()
    assert choice.get_absolute_url() == choice.question.poll.get_absolute_url()
    assert vote.get_absolute_url() == vote.choice.question.poll.get_absolute_url()
    assert (
        other_vote.get_absolute_url()
