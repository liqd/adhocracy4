...
poll_component = components.modules.get("open_poll")
export_poll_component = components.modules.get("open_poll_export")
...
def test_poll_edit_view(client, phase_factory, user_factory):
    ...
    assert_template_response(response, "a4open_poll/open_poll_dashboard.html")
    assert response.template_name[0] == "a4open_poll/open_poll_dashboard.html"
...
def test_poll_dashboard_export_view(client, phase_factory, user_factory):
    ...
    assert response.context["poll_export"] == reverse(
        "a4dashboard:open-poll-export", kwargs={"module_slug": module.slug}
    )
    assert "comment_export" in response.context
    assert response.context["comment_export"] == reverse(
        "a4dashboard:open-poll-comment-export", kwargs={"module_slug": module.slug}
    )
...
def test_poll_export_view(
    ...
    url = reverse("a4dashboard:open-poll-export", kwargs={"module_slug": module.slug})
