...
@pytest.mark.django_db
def test_authenticated_user_rating_list(apiclient, user, question, question_ct):
    url = reverse(
        "ratings-list",
        kwargs={"content_type": question_ct.pk, "object_pk": question.pk},
    )
...
@pytest.mark.django_db
def test_authenticated_user_can_edit_own_rating(rating, question_ct, apiclient):
...
@pytest.mark.django_db
def test_authenticated_user_can_rating_higher_1(
    rating, question, question_ct, apiclient
):
    question = rating.content_object
    apiclient.force_authenticate(user=rating.creator)
    data = {"value": 10}
...
@pytest.mark.django_db
def test_authenticated_user_can_rating_lower_minus1(
    rating, question, question_ct, apiclient
):
    question = rating.content_object
    apiclient.force_authenticate(user=rating.creator)
...
@pytest.mark.django_db
def test_meta_info_of_rating(
    rating_factory, question, question_ct, apiclient, user, another_user
):
