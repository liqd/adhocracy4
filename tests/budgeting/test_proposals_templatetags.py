...
def test_react_proposals(module, rf):
    ...
    props = get_rendered_props({'request': request, 'module': module},
                               expected,
                               template)
    assert props == {
...
@pytest.mark.django_db
def test_react_proposals_vote(phase_factory, proposal_factory,
                              voting_token_factory, token_vote_factory, user,
                              rf):
    ...
    template = '{% load react_proposals_vote %}' \
               '{% react_proposals_vote module proposal %}'
    contenttype = ContentType.objects.get_for_model(Proposal)
    expected = (
    ...
    with freeze_phase(phase):
        ...
        props = get_rendered_props({'request': request,
                                    'module': module,
                                    'proposal': proposal},
                                   expected,
                                   template)
    ...
    with freeze_phase(phase):
        ...
        assert props == {
            ...
            'token_info': {'votes_left': True,
                           'num_votes_left': token.allowed_votes - 1}
        }


@pytest.mark.django_db
def test_react_support(phase_factory, proposal_factory, rating_factory, user,
                       rf):
    phase, module, project, proposal = setup_phase(phase_factory,
                                                   proposal_factory,
                                                   phases.SupportPhase)
    ...
    with freeze_phase(phase):
        proposal = get_annotated_proposal(module)
        props = get_rendered_props({'request': request, 'proposal': proposal},
                                   expected,
                                   template)
        assert props == {
