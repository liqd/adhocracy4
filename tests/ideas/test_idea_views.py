...
def test_detail_view(client, phase, idea):
    url = reverse('ideas:idea-detail', kwargs={'slug': idea.slug})
...
def test_create_view(client, phase, user):
    ...
    with freeze_time(phase.start_date):
        ...
        assert count == 0
        url = reverse('ideas:idea-create', kwargs={'slug': module.slug})
        response = client.get(url)
        assert response.status_code == 302
        assert redirect_target(response) == 'account_login'
...
def test_update_view(client, phase, idea):
    ...
    user = idea.creator
    with freeze_time(phase.start_date):
        url = reverse('ideas:idea-update', kwargs={'slug': idea.slug})
        response = client.get(url)
        assert response.status_code == 302
        client.login(username=user.email, password='password')
        response = client.get(url)
...
def test_delete_view_wrong_user(client, phase, idea, user, user2):
    ...
    with freeze_time(phase.start_date):
        ...
        url = reverse('ideas:idea-delete', kwargs={'slug': idea.slug})
