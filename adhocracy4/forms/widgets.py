...
class DateTimeInput(form_widgets.SplitDateTimeWidget):
    def __init__(self, time_label="", time_default=None, *args, **kwargs):
        ...
        self.time_default = time_default or datetime.time(
            hour=0, minute=0, tzinfo=tzinfo
        )
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        time_attrs.update(
            {
                "class": "timepicker",
                "placeholder": self.widgets[1].format_value(self.get_default_time()),
                "id": attrs["id"] + "_time",
                "required": False,
            }
        )
        ...
        return render_to_string(
            ...
            {
                "date": self.widgets[0].render(name + "_0", date, date_attrs),
                "time": self.widgets[1].render(name + "_1", time, time_attrs),
                "time_label": {
    ...
    def get_default_time(self):
        ...
        if not self.time_default:
            return time_widget.format_value(
                datetime.time(hour=0, minute=0, tzinfo=tzinfo)
            )
