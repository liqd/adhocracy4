...
from adhocracy4.open_poll import validators
from adhocracy4.polls.models import ChoiceQuerySet


class QuestionQuerySet(models.QuerySet):
    def annotate_vote_count(self):
        return self.annotate(
            vote_count=models.Count("choices__votes", distinct=True),
            vote_count_multi=models.Count("choices__votes", distinct=True),
            answer_count=models.Count("answers", distinct=True),
        ).order_by("weight")


class OpenPoll(module_models.OptionalUserItem):
...
class Question(models.Model):
    ...
    poll = models.ForeignKey(
        "openpoll", on_delete=models.CASCADE, related_name="questions"
    )
