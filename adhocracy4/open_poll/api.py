...
class OpenPollViewSet(
    mixins.RetrieveModelMixin, mixins.UpdateModelMixin, viewsets.GenericViewSet
):
    queryset = OpenPoll.objects.all()
            ...
            if question.is_open:
                ...
                if open_answer:
                    Answer.objects.create(
                        question=question, answer=open_answer, creator=creator
                    )
            else:
                self.clear_current_choices(question)
                for choice in choices:
                    vote = Vote.objects.create(choice_id=choice.id, creator=creator)
