...
from adhocracy4.open_poll import models as poll_models

User = get_user_model()


class OpenPollCommentExportView(
    ...
    permission_required = "a4open_poll.change_openpoll"
    ...
    def get_queryset(self):
        comments = Comment.objects.filter(
            open_poll__module=self.module
        ) | Comment.objects.filter(parent_comment__open_poll__module=self.module)
        return comments
...
class OpenPollExportView(PermissionRequiredMixin, export_views.BaseItemExportView):
    permission_required = "a4open_poll.change_openpoll"
    ...
    @property
    def poll(self):
        return poll_models.OpenPoll.objects.get(module=self.module)
