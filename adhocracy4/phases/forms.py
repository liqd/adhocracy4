...
class PhaseInlineFormSet(BaseInlineFormSet):
    def clean(self):
        ...
        for form in self.forms:
            if (
                "start_date" in form.cleaned_data
                ...
                if phase_dates:
                    for phase_date in phase_dates:
                        if start_date < phase_date[1] and phase_date[0] < end_date:
                            msg = _(
                                "Phases cannot run at the same time "
                                "and must follow after each other."
                            )
                            form.add_error("end_date", msg)
                        if (start_date < phase_date[0] and weight > phase_date[2]) or (
                            start_date > phase_date[0] and weight < phase_date[2]
                        ):
                            msg = _("Phases need to be in same order " "as in form.")
