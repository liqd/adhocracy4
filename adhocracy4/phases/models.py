...
class PhasesQuerySet(models.QuerySet):
    ...
    def past_phases(self):
        ...
        return self.filter(end_date__lte=timezone.now()).order_by("start_date")
    ...
    def future_phases(self):
        ...
        """
        return self.filter(
            models.Q(start_date__gt=timezone.now()) | models.Q(start_date=None)
    ...
    def past_and_active_phases(self):
        ...
        return self.filter(start_date__lte=timezone.now()).order_by("start_date")
...
class Phase(models.Model):
        ...
        if self.end_date and self.start_date:
            if self.end_date < self.start_date:
                raise ValidationError(
                    {"end_date": _("End date can not be before " "start date.")}
                )
    ...
    def starts_first_of_project(self):
        ...
        if not self.module.is_draft:
            phases = self.module.project.phases.filter(module__is_draft=False).order_by(
