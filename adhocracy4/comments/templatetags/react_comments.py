...
def react_comments(context, obj):
    ...
    serializer = ThreadSerializer(
        obj.comments.all().order_by("-created"), many=True, context={"request": request}
    )
    ...
    would_have_comment_permission = NormalUser().would_have_perm(permission, obj)
    ...
    attributes = {
        ...
        "user_name": user_name,
        "isReadOnly": (
            not has_comment_permission and not would_have_comment_permission
        ),
        "isContextMember": (
            is_context_member(user, obj) or is_context_member(NormalUser(), obj)
        ),
    }
