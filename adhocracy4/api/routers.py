...
class CustomRouterMixin:
    ...
    def routes(self):
        routes = super().routes
        return [
            route._replace(url=route.url.replace("{prefix}", self.prefix_regex))
            for route in routes
        ]
...
class ModuleRouterMixin(CustomRouterMixin):
    ...
    prefix_regex = r"modules/(?P<module_pk>[\d]+)/{prefix}"
...
class OrganisationRouterMixin(CustomRouterMixin):
    ...
    prefix_regex = r"organisations/(?P<organisation_pk>[\d]+)/{prefix}"
...
class OrganisationDefaultRouter(OrganisationRouterMixin, routers.DefaultRouter):
    pass


class CommentRouterMixin(CustomRouterMixin):

    prefix_regex = r"comments/(?P<comment_pk>[\d]+)/{prefix}"
