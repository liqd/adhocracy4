...
class BlueprintListView(mixins.DashboardBaseMixin, generic.TemplateView):
...
class ProjectUpdateView(generic.RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        project = get_object_or_404(project_models.Project, slug=kwargs["project_slug"])
        dashboard = get_project_dashboard(project)
        component = dashboard.get_project_components()[0]
        return component.get_base_url(project)


class ProjectPublishView(
    ProjectMixin, mixins.DashboardBaseMixin, SingleObjectMixin, generic.View
):
    ...
    def publish_project(self):
        ...
        if project.published_modules.count() == 0:
            messages.error(
                self.request, _("Project cannot be published. " "No module is added.")
            )
            return
        ...
        if not is_complete:
            messages.error(
                self.request,
                _("Project cannot be published. " "Required fields are missing."),
            )
            return

        responses = signals.project_pre_publish.send(
            sender=None, project=project, user=self.request.user
        )
        ...
        project.save()
        signals.project_published.send(
            sender=None, project=project, user=self.request.user
        )

        messages.success(self.request, _("the project has been published."))
    ...
    def unpublish_project(self):
        ...
        project.save()
        signals.project_unpublished.send(
            sender=None, project=project, user=self.request.user
        )
        messages.success(self.request, _("the project has been unpublished."))
