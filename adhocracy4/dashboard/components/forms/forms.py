...
class ProjectDashboardForm(ImageMetadataMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.get_project().is_draft:
            _make_fields_required(self.fields.items(), self.get_required_fields())
    ...
    def clean_description(self):
        # description not on all forms which inherit
...
class ModuleDashboardFormSet(forms.BaseInlineFormSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for form in self.forms:
            _make_fields_required_for_publish(form.fields.items(), required_fields)
            if not self.instance.project.is_draft:
                _make_fields_required(form.fields.items(), required_fields)
